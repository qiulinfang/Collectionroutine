		...
		<view class="community">
			...
			<view class="content">
				<view class="generalRecommendations" v-if="currentTab === 0">
					<articleItroduction :items = 'pageList'></articleItroduction>
...
<script>
	...
	export default {
		...
		methods: {
			async fetchData() {
				try {
					const url = `http://localhost:3000/api/IntactArticle/${this.currentTab}`; // JSON Server的URL
					const res = await uni.request({
						url,
						method: 'GET',
					});
					this.pageList = res.data; // 假设JSON Server返回的是一个数组
					console.log('获取数据成功')
				} catch (error) {
